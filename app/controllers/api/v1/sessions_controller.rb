class Api::V1::SessionsController < Api::V1::BaseController
  ...
  swagger_path '/api/v1/sessions/signup' do
    operation :post do
      key :summary, 'User signup'
      key :description, 'Returns a single user if the user has access'
      key :tags, [
        'session'
      ]
      parameter do
        key :name, :email
        key :in, :query
        key :description, 'email of user to signup'
        key :required, true
        key :type, :string
        key :format, :email
      end
      parameter do
        key :name, :name
        key :in, :query
        key :description, 'name of user to signup'
        key :required, true
        key :type, :string
        key :format, :email
      end
      parameter do
        key :name, :password
        key :in, :query
        key :description, 'password of user to signup'
        key :required, true
        key :type, :string
      end
      response 200 do
  ...
  def signup
    ...
    if @user.save
      ...
      api_error(messages: I18n.t('errors.signup_failed_has_email'), status: 401)
    end
